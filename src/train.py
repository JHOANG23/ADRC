...
from src.data_split import MixedBatchIterator, balance_data, custom_balanced_2_fold
...
def pre_train(device, embeddings, labels, en_train_df, es_val_loader, output_path):
    ...
    for i in range(5):
        ...
        y_full = en_train_df['binary_label']
...
def get_es_tensors(es_df_subset, es_csv_df, embeddings, labels):
    ...
    global_indices = es_csv_df[es_csv_df['file_name'].isin(subset_files)].index.values.nonzero()[0]
...
def split_es_val_test(es_df, seed):
    es_ad = es_df[es_df['binary_label'] == 0].copy().reset_index(drop=True)
    es_nc = es_df[es_df['binary_label'] == 1].copy().reset_index(drop=True)

    # Shuffle for reproducibility
    es_ad = es_ad.sample(frac=1, random_state=seed).reset_index(drop=True)
    es_nc = es_nc.sample(frac=1, random_state=seed).reset_index(drop=True)

    # Take 3 of each for validation/sample set
    val_ad = es_ad.iloc[:3]
    val_nc = es_nc.iloc[:3]
    es_val = pd.concat([val_ad, val_nc]).sample(frac=1, random_state=seed).reset_index(drop=True)

    # Remaining goes to test set
    test_ad = es_ad.iloc[3:]
    test_nc = es_nc.iloc[3:]
...
def prep_loaders(df, en_csv_df, es_csv_df, seed):
    # helper to label and filter by language
    def relabel(subset):
        subset['binary_label'] = subset['synd2'].apply(lambda x: 1 if x == 2 else 0)
        return subset
    
    en_train = relabel(balance_data(df[df['language'] == 'en'].copy(), seed))
    es_df = relabel(df[df['language']=='es'].copy())
    es_val, es_test = split_es_val_test(es_df, seed)

    # merges for embedding alignment
    def merge_split(csv_df, split_df):
        return csv_df.merge(split_df[['file_name', 'binary_label']], on='file_name', how='inner')
    ...
    # return en_split_df, es_split_df, test_split_df, es_val
...
def complete_train(device, embeddings, labels, name, seed):
    ...
    en_train_df, es_val_df, es_test_df = prep_loaders(df, en_csv_df, es_csv_df, seed)
...
def main():
    ...
    labels_en = torch.where(labels_en==2, 1, 0)   # binary map
    labels_es = torch.where(labels_es==2, 1, 0)   # binary map

    embeddings = (embeddings_en, embeddings_es)
    labels = (labels_en, labels_es)
    names = ['1_hometown', '2_hometown', '3_hometown', '4_hometown', '5_hometown']
