...
def extract_prosodic_features(input_df: pd.DataFrame, num_intervals: int = 10):
    ...
    for row in input_df.itertuples(index=False):
        ...
        overall_std_f0 = call(pitch, "Get standard deviation", 0, 0, "Hertz")
        ...
        jitter_ppq5 = call(point_proc, "Get jitter (ppq5)", 0, 0, 0.0001, 0.02, 1.3)
        jitter_ddp = call(point_proc, "Get jitter (ddp)", 0, 0, 0.0001, 0.02, 1.3)
        ...
        shimmer_apq11 = call([snd, point_proc], "Get shimmer (apq11)", 0, 0, 0.0001, 0.02, 1.3, 1.6)
        shimmer_apq3 = call([snd, point_proc], "Get shimmer (apq3)", 0, 0, 0.0001, 0.02, 1.3, 1.6)
        shimmer_apq5 = call([snd, point_proc], "Get shimmer (apq5)", 0, 0, 0.0001, 0.02, 1.3, 1.6)
        shimmer_dda = call([snd, point_proc], "Get shimmer (dda)", 0, 0, 0.0001, 0.02, 1.3, 1.6)
        ...
        # Speech rate measures (requires TextGrid or voiced segments)
        ...
        feature_dict = {
            ...
            "LTAS_1_to_3_kHz_Pa": ltas_1to3_pa,
            "LTAS_1_to_3_kHz_dB": ltas_1to3_db,
            "voicedcount": voicedcount,
            "npause": npause,
            "speakingrate": speakingrate,
            "articulationrate": articulationrate,
            "asd": asd,
            "hnr": hnr,
            "mean_f0": overall_mean_f0,
            "min_f0": min_f0,
            "max_f0": max_f0,
            "std_f0": overall_std_f0,
        }

        # Add interval F0
        for i, val in enumerate(interval_means):
            feature_dict[f"Mean_f0_Interval_{i+1}"] = val
